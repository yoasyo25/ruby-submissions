...
- Strong work, especially in pushing logic down the stack to the models
- Could not seed your database, the job needed a category as well. Check this with a friend before submitting to be sure it works
- A shortcut for `company = Company.find(params[:id) company.destroy` is `Company.destroy(params[:id])`
- `@category = Category.find(params[:id])
    @category.update(category_params)` could become `@category =
   Category.update(params[:id], category_params)` is a shortcut
- `top_three` has some ruby that could be accomplished with AR
- Some of the user interaction is a little confusing and needs to be polished to be user ready. Editing a category confused me but overall, nice work styling
- Make sure to run test suite before submittal as there is one failing test, a working dashboard test would have pushed testing to a 4
