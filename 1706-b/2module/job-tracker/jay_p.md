...
- Routes should follow conventions. For example, our jobs lcation action might be better going to an index action on a JobAnalyticsController or soemthing to be clear that this will show all the analytics for the jobs. Index = all things. A custom action does not follow REST conventions
...
- Don't see full functionality with the route `job?sort=location` etc.
- ActiveRecord is incomplete
- Cannot navigate from root page
- Cannot run test suite due to error that I don't want to have to resolve, this should be resolved by running it with another student
- Testing is inconsistent. No tests for contact features and no sad paths. Also not testing model level methods.
